//
//  ModifiedJiroTextField.swift
//  TextFieldEffects
//
//  Created by Raúl Riera on 24/01/2015.
//  Modified by Mislav Žabčić on  16/05/2016
...
@IBDesignable public class JiroTextField: TextFieldEffects {
    ...
    private let placeholderInsets = CGPoint(x: 8, y: 12)
    private let placeholderEmpty = CGPoint(x: 8, y: 5)
    private let textFieldInsets = CGPoint(x:8, y:19)
    ...
    override func drawViewsForRect(rect: CGRect) {
        ...
        normalFont = placeholderLabel.font
    ...
    private func normalFontSize() -> UIFont! {
        let font = normalFont as UIFont!
        let rFont = UIFont(name: font.fontName, size: font.pointSize)
        return rFont
    }
    ...
    private func layoutPlaceholderInTextRect() {
        ...
        placeholderLabel.frame = CGRect(x: originX, y: textRect.size.height/2.8,
    ...
    override func animateViewsForTextDisplay() {
        placeholderLabel.font = placeholderFontFromFont(font!)
        if text!.isEmpty {
            placeholderLabel.font = normalFontSize()
            placeholderLabel.frame.size.width = 200;
